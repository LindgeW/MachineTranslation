...
class AttMTDecoder(nn.Module):
    def __init__(self, args, embedding_weights=None):
        ...
        self._decoder = nn.GRU(input_size=args.tgt_embedding_dim + args.hidden_size,
        ...
        self.linear = nn.Linear(in_features=args.hidden_size * 2,
    ...
    def forward(self, inputs, init_hidden, encoder_outputs=None):
        ...
        # 计算encoder输出的注意力权重分数
        hidden = init_hidden[-1].unsqueeze(0)  # 以最后一层的隐层状态作为query
        # (bz, 1, hidden_size) * (bz, hidden_size, seq_len) -> (bz, 1, seq_len) -> (batch_size, seq_len)
        att_weights = torch.bmm(hidden.transpose(0, 1), encoder_outputs.transpose(1, 2)).squeeze(dim=1)
        att_weights /= math.sqrt(encoder_outputs.size(-1))
        soft_att_weights = F.softmax(att_weights, dim=1)
        # 将权重分数施加到encoder输出上
        # (bz, 1, seq_len) * (bz, seq_len, hidden_size) -> (bz, 1, hidden_size)
        dec_context = torch.bmm(soft_att_weights.unsqueeze(dim=1), encoder_outputs)
        # 整合t-1时刻decoder的输出和encoder的局部输出
        # (bz, 1, hidden_size + embed_dim)
        rnn_input = torch.cat((embed, dec_context), dim=2)

        # rnn_out: (bz, 1, hidden_size * nb_directions)
        # final_hidden: (num_layers * nb_directions, bz, hidden_size)
        rnn_out, final_hidden = self._decoder(rnn_input, init_hidden)

        # (bz, hidden_size * 2) -> (bz, tgt_vocab_size)
        out = self.linear(torch.cat((rnn_out.squeeze(), dec_context.squeeze()), dim=1))
